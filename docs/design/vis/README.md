...
- **System Footprint Flamegraph**：按 prompt/subagent tag 和执行栈聚合系统 effect。
- **Token / Reasoning Flamegraph**：按 prompt/subagent/LLM-call tag 聚合 token、latency 和 cost。
- **Behavior Diff Flamegraph**：比较两个 run、model、prompt 或 workflow 的聚合 footprint 差异。
...
- [Intent-to-Effect Flame Graph](intent-to-effect-flame-graph.md)：早期火焰图草案，已被 one-word semantic tags + execution-stack aggregation 方案取代。
...
- [One-Word Semantic Tags And Task-Aligned Aggregation](one-word-semantic-tags.md)：用本地小模型给 session/prompt/subagent/LLM call 生成单词标签，并把标签接到系统 footprint、token flamegraph 和 behavior diff 上。
