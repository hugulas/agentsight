# Agent Filesystem Evidence Plane: 给 AI Coding Agent 的文件系统行为可视化
...
> 这个 agent 对文件系统做了什么，而且这些动作能不能被解释、归因和审计？

所以这里的文件系统不是一个普通资源树，而是一个 **agent 行为的证据平面**。
...
AgentFS、Mirage 这类项目则把文件系统本身变成 agent interface。AgentFS 强调 state、audit trail、time travel、sandbox 互补；Mirage 把 S3、Google Drive、Slack、Gmail、GitHub、Redis 等后端挂成统一虚拟文件系统，让 agent 用 `ls`、`grep`、`cat`、`cp` 这套 Unix 词汇工作。它们很接近底层抽象，但主要目标是 access、state、persistence 或 sandbox，不是把 agent runtime 文件行为画成可审计图。
...
```text
Agent run = semantic trace + process tree + filesystem provenance + policy verdict
```
...
LLM span
  -> tool call
    -> process tree
      -> syscall / fs event
        -> file object / directory object / external mount
          -> diff / snapshot effect
            -> policy verdict
```

这不是一个“漂亮 UI”问题。真正的系统贡献是：定义证据模型、低侵入采集、跨层归因、可查询的行为图，以及几种能一眼回答问题的可视化。

## 设计原则

第一，文件路径不是分类标签，而是证据坐标。
...
最好的图应该可以贴进 PR、incident report、blog、security review，不依赖用户打开完整 dashboard。

第五，每张图只回答一个主问题。

不要把所有指标塞进一张超级图。agent 文件行为有多个场景：review、debug、incident、policy、sandbox、multi-agent conflict。每个场景需要自己的主图。
...
- recovery context
- 判断某个写入是否符合用户任务
- 对 prompt injection 后的异常写入做证据重建
...
这张图不要把重点放在“自然语言 rule 解析得多聪明”。重点应该是：AgentSight 用 runtime evidence 证明了哪些路径被读、写、删、上传，以及这些动作在 policy lens 下是什么 verdict。
...
当 evidence 和 attribution 稳定后，再把规则投影到图上。这样 policy 不是空泛的 guardrail，而是对实际 runtime behavior 的 verdict。
...
底层 runtime evidence 可以来自：

- AgentSight eBPF process/file/network tracing
- fanotify / auditd / inotify
- syscall events
- process tree
- git index and diff
...
> A filesystem evidence plane for AI coding agents.
...
> Git diff shows the patch. Agent filesystem evidence shows the run that produced the patch.
