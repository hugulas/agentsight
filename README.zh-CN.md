...
# 或下载 GitHub Release 里的 Linux 预编译二进制：
wget https://github.com/eunomia-bpf/agentsight/releases/latest/download/agentsight && chmod +x agentsight
```

**最简单的方式 —— `stat`/`record` 直接启动你的智能体并自动追踪（零配置）：**

```bash
# 把你的智能体命令写在 `stat --` 或 `record --` 后面即可。AgentSight 会自动发现要 hook 的二进制
# （解析符号链接 / 包装脚本）、推导进程过滤器、启动智能体，并在其退出时停止追踪。
sudo ./agentsight stat -- claude
sudo ./agentsight record -- claude -p "总结这个仓库"
sudo ./agentsight record -- python my_agent.py
sudo ./agentsight record -- gemini
```

智能体会正常在你的终端中运行，监控在后台进行。
访问 [http://127.0.0.1:7395](http://127.0.0.1:7395) 实时查看捕获的数据。

**手动方式 —— `record` 按进程过滤器附加（当智能体在别处启动时使用）：**
...
sudo ./agentsight record --db record.db --adapter auto -- claude -p "总结这个仓库"

# 查询或导出捕获结果，供 Dashboard / 静态 demo 使用
./agentsight stat --db record.db
./agentsight top --db record.db --once
./agentsight report --db record.db
./agentsight db token --db record.db --group-by model
./agentsight db audit --db record.db --json
./agentsight db export --db record.db --output trace.agentsight.json
```

`./agentsight top` 不带 `--db` 时是实时 session 视图，会自动发现 Claude/Codex 本地会话，并把可匹配的 agent root PID 及其子进程族挂到 session 上显示 CPU/RSS/token/tool/prompt；`./agentsight top --db record.db --once` 用于查看已保存 session 的 token、进程、文件和网络热区。

内置 SQL adapter 目前覆盖 Anthropic、Claude Code、Gemini CLI 和 OpenClaw 风格会话。使用 `--no-adapters` 可以只保留通用 SQLite 投影；使用 `agentsight db adapters list --json` 可以查看可用 adapter。
